            ...
            if (transportRequests && transportRequests.length > 0) {
              for (const tr of transportRequests) {
                try {
                  ...
                  console.log(`[cloudbeds-reconcile] Sent cancellation notification for transport ${tr.id}`);
                } catch (notifErr) {
                  console.error(`[cloudbeds-reconcile] Failed to send cancellation notification for transport ${tr.id}:`, notifErr);
