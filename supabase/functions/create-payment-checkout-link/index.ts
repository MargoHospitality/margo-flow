...
interface CreatePaymentCheckoutLinkBody {
  riad_id?: string;
...
serve(async (req) => {
  ...
  try {
    ...
    const body = (await req.json()) as CreatePaymentCheckoutLinkBody;
    if (!body.riad_id || !body.reservation_id || !body.check_in_date || typeof body.amount !== "number" || !body.client_whatsapp) {
      return jsonResponse({
        success: false,
        error: "riad_id, reservation_id, check_in_date, amount, and client_whatsapp are required",
      }, 400);
    }
    ...
    const normalizedWhatsapp = normalizeWhatsappNumber(body.client_whatsapp);
    ...
    try {
      ...
      const nextStatus = whatsappResult.success ? "checkout_link_sent" : "checkout_link_created";
      const sentAt = whatsappResult.success ? new Date().toISOString() : null;

      const { error: whatsappUpdateError } = await adminClient
        .from("reservation_payments")
        .update({
          status: nextStatus,
          client_whatsapp: normalizedWhatsapp,
          ...
          link_sent_count: whatsappResult.success ? 1 : 0,
    ...
    console.error("[create-payment-checkout-link]", error);
    return jsonResponse({
      success: false,
      error: error instanceof Error ? error.message : "Unexpected error while creating the payment link",
    }, 500);
  }
});
