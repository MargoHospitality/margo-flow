...
import { Loader2, LogOut, Bell, Shield, AlertCircle, Search, Calendar, Clock, CalendarDays, List, XCircle, Eye, EyeOff, CreditCard, MessageSquareText, CarFront, Users } from 'lucide-react';
...
export default function Backoffice() {
  ...
  const [showCancelledRejected, setShowCancelledRejected] = useState(false);
  ...
  const filteredRequests = useMemo(() => {
    ...
    switch (activeTab) {
      ...
      case 'pending':
        ...
        break;
      case 'all':
        // Show upcoming transports (today and future)
        // By default, exclude cancelled/rejected unless toggle is on
        filtered = requests.filter(r => {
          const date = parseISO(r.transport_date);
          ...
          // Filter out cancelled/rejected unless toggle is on
          if (!showCancelledRejected) {
            const cancelledStatuses = ['rejected', 'canceled', 'cancelled', 'canceled_due_to_reservation'];
            return !cancelledStatuses.includes(r.status);
          }
      ...
      <main className="flex-1 container mx-auto px-4 py-6">
        <div className="space-y-6">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {/* Search Toggle and Cancelled/Rejected filter */}
            <div className="ml-auto flex items-center gap-2">
              {activeTab === 'all' && (
                <Button
                  variant={showCancelledRejected ? 'secondary' : 'ghost'}
                  size="sm"
                  onClick={() => setShowCancelledRejected(!showCancelledRejected)}
                  className="gap-2"
                >
                  {showCancelledRejected ? (
                    <>
                    ...
                    <>
                      ...
                      <span className="hidden sm:inline">Show cancelled</span>
                    </>
                  )}
                </Button>
              )}
