...
export function ReservationEntry({ onReservationFound, preselectedRiadId }: ReservationEntryProps) {
  ...
  console.log('[ReservationEntry] Render state:', {
    reservationId,
    checkInDate,
    selectedRiad: selectedRiad?.name
  });
  ...
  // Pre-fill from URL params on mount (once)
  useEffect(() => {
    const params = new URLSearchParams(window.location.search);
    const reservationParam = params.get('reservation');
    const checkinParam = params.get('checkin');

    console.log('[ReservationEntry] URL params:', {
    ...
    if (reservationParam) {
      console.log('[ReservationEntry] Setting reservation ID:', reservationParam);
    ...
    if (checkinParam) {
      try {
        ...
        const parsedDate = parse(checkinParam, 'yyyy-MM-dd', new Date());
        if (!isNaN(parsedDate.getTime())) {
          console.log('[ReservationEntry] Setting check-in date:', parsedDate);
          setCheckInDate(parsedDate);
        } else {
          console.error('[ReservationEntry] Invalid parsed date:', parsedDate);
        }
      } catch (error) {
        console.error('[ReservationEntry] Date parsing error:', error);
      }
    }
  }, []);

  // Pre-select riad after riads are loaded
  useEffect(() => {
    if (preselectedRiadId && riads.length > 0) {
      // Try to find by Cloudbeds property ID first (for Guest App integration)
      let riad = riads.find(r => r.cloudbeds_property_id === preselectedRiadId);
      
      // Fallback: try to find by UUID (for backward compatibility)
      if (!riad) {
        riad = riads.find(r => r.id === preselectedRiadId);
      }
      
      if (riad) {
        console.log('[ReservationEntry] Pre-selecting riad:', riad.name, 'by', 
          riad.cloudbeds_property_id === preselectedRiadId ? 'Cloudbeds ID' : 'UUID');
        setSelectedRiad(riad);
        setRiadSearch(riad.name);
      } else {
        console.warn('[ReservationEntry] No riad found for preselectedRiadId:', preselectedRiadId);
      }
    }
